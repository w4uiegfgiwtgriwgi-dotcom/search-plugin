...
> 日期：2026-08-06
> 范围：阶段2截图反查最小闭环、文件上传、基础鲁棒性测试、批量候选匹配、局部精排、阈值 UI 和匹配帧收藏
...
- API 阶段2单元测试：6 个通过。
- FastAPI 路由加载检查：通过，已注册上传、批量匹配、匹配帧收藏接口。
...
- 项目页尚未做统一素材视图，目前阶段1搜索收藏和阶段2匹配帧收藏还分属不同接口。
