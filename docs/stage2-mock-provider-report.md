...
> 日期：2026-08-06
> 范围：阶段2截图反查最小闭环、文件上传、基础鲁棒性测试、批量候选匹配、局部精排、阈值 UI、匹配帧收藏和统一项目素材视图
...
- FastAPI 路由加载检查：通过，已注册上传、批量匹配、匹配帧收藏和统一项目素材接口。
...
- 统一项目素材视图目前只做列表展示，尚未支持筛选、排序、删除和批量导出。
